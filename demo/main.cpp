...
int main(int argc, char*argv[]) {
	...
	{ // This scope is for the purpose of removing false memory leak positives
		...
		model.setup(parser.getAgents(), parser.getWaypoints(), Ped::TASK);
		...
		if (timing_mode)
		{
			...
			double fps_seq, fps_target;
			{
				Ped::Model model;
				ParseScenario parser(scenefile);
				model.setup(parser.getAgents(), parser.getWaypoints(), Ped::MOVESEQ);
			...
			Ped::IMPLEMENTATION implementation_to_test = Ped::TASK;
			{
				Ped::Model model;
				ParseScenario parser(scenefile);
